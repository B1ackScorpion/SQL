...
SELECT
	t.surname,
    l.course
FROM teacher t -- имя_таблицы псевдоним
...
-- Вывести всех учителей, которые проводят уроки по Вебу
SELECT
	t.surname,
    l.course
FROM teacher t
...
-- Подзапросы
-- Выведем название дожностей, ср.ЗП которых выше средней по пред-ю
SELECT *
FROM lesson_3.staff;

SELECT
	AVG(salary)
FROM lesson_3.staff; -- 40923.076923
...
-- Выборка данных по пользователю: ФИО, город проживания, название аватарки 
SELECT
	u.firstname,
    u.lastname,
    p.hometown AS city,
    m.filename AS avatar_name
FROM users u
Join `profiles` p ON u.id = p.user_id
JOIN media m ON p.photo_id = m.id;

-- Выборка данных по пользователю: ФИО, город проживания, название ФОТОГРАФИЙ
SELECT
	u.firstname,
    u.lastname,
    p.hometown AS city,
    m.filename AS photo_name,
    mt.name_type AS type_media
FROM users u
Join `profiles` p ON u.id = p.user_id
JOIN media m ON m.user_id = u.id -- Получить все МЕДИА (не только фото!)
JOIN media_types mt ON mt.id = m.media_type_id -- соединяем, чтобы получить тип медиафайла
WHERE mt.name_type = 'Photo'; -- Выбрали именно тип медиа, равный "фото"
